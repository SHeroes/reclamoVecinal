	...
	if($reclamos_list != ''){
      ...
      foreach ($reclamos_list as $rec) {
      	if ($rec['estado_servicio'] == 1){
      		$estado = '<td><div class="btn btn-success">OK</div></td>';
      	} else if ( $rec['horas_transcurridas'] > 24) {
	      		$estado = '<td><div class="btn btn-danger">URGENTE</div></td>';
	      	} else {
	      		$estado = '<td><div class="btn btn-warning">ATENCION</div></td>';
      	}
		
        echo  '<tr class="reclamo_row"><th scope="row" class="" id_reclamo="'.$rec['id_reclamo'].'"value="'. $rec['id_vecino'].' ">'. $rec['codigo_reclamo'] .'</th>'.
        	$estado.
...
<script type="text/javascript" src="https://maps.google.com/maps/api/js?key=AIzaSyA0GuxesQUCN_PpC-x-RnnOZaxBzeTdqN0">  </script>
